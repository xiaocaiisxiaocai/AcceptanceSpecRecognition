...
2. **表达方式不同**：改变句式、用词、语序，像"你吃饭了吗"改成"你用餐了吗"
3. **保留关键信息**：数值、单位、品牌必须保留
4. **自然流畅**：符合中文表达习惯
...
function Invoke-Paraphrase {
    ...
    $prompt = $promptTemplate -replace '\{ORIGINAL_TEXT\}', $text
...
foreach ($spec in $specs) {
    ...
    Write-Host "🔄 [$index/$($specs.Count)] 改写: $($spec.project.Substring(0, [Math]::Min(20, $spec.project.Length)))..." -ForegroundColor Cyan

    $paraphrased = Invoke-Paraphrase -text $spec.specification

    if ($paraphrased -and $paraphrased -ne $spec.specification.Trim()) {
