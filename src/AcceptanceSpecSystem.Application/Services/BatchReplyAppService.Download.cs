...
{
    public Task<MatchingDownloadResult> DownloadAsync(BatchReplyUserContext user, string taskId, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var content = _fileStorage.OpenReadStream(artifact.RelativePath);
            return Task.FromResult(new MatchingDownloadResult(content, artifact.ContentType, artifact.FileName));
        }
