...
public sealed class ImportDuplicateDetectionService
{
    ...
    public ImportDuplicateDetectionService(
        ...
        IImportEmbeddingCache? specEmbeddingCacheService = null)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var cachedEmbeddings = await _specEmbeddingCacheService.GetImportDuplicateEmbeddingsAsync(
                existingSpecs,
                normalizedOptions.EmbeddingServiceId,
                cancellationToken);
            foreach (var candidate in candidates)
            {
                if (cachedEmbeddings.TryGetValue(candidate.Spec.Id, out var embedding))
                {
