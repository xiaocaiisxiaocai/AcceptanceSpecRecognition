...
- 引入配置化表格路由规则：由人工配置、AI 建议或用户确认学习产生表格类型与推荐级别，不在后端写死客户业务词。
- 引入文档级候选表排序：优先推荐结构完整、规则或历史信号更强的主表或专项验收表，降低已配置辅助表的干扰。
- 将用户确认结果沉淀为“结构案例”和客户级学习规则，按客户、表名、表头相似度、最近确认时间和使用次数为后续识别加权。
- LLM 结构裁决预算优先分配给最有价值的灰区候选表，而不是简单按表格遍历顺序消耗。
- 前端确认卡展示推荐导入、可选确认、建议跳过分组，并明确 NeedConfirm 或 Skip 的原因。
- 前端提供智能结构路由规则配置页，支持人员手写、启停、客户域隔离和学习规则维护。
...
- `DocumentTemplate` / 可能新增结构案例字段或轻量实体
  - `SmartStructureRoutingRule`
  - `SmartConfigController`
  - `SmartStructureRoutingRulesController`
  - `web/src/api/smart-config`
  - `web/src/api/smart-structure-routing-rules`
  - `SmartStructureConfirmCard.vue`
  - `web/src/views/config/smart-structure-routing-rules`
