...
- 当前最佳候选存在字段文本差异，但没有硬冲突。
- 当前结果低于高置信阈值、被标记为人工确认、或详情中存在仅文本表现层面的差异。
- 已启用 LLM 复核或新增“AI 等价裁决”开关。

裁决流程：

1. 正常执行 Embedding 召回与多阶段证据重排。
2. 对满足边界条件的 Top1 候选调用 AI 等价裁决 Prompt。
3. 要求 AI 返回严格 JSON。
