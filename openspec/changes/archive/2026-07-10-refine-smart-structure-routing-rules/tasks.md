...
- [x] 4.5 补前端测试：页面默认值、不暴露固定类型枚举和 TableName 兜底说明。
- [x] 4.6 补后端测试：历史 `Learned` 表格路由规则不展示、不生效。
