...
test("AI服务配置页提供禁用切换且禁用后不可测试或探测模型", () => {
  ...
  assert.match(aiServicesTableSource, /row\.isDisabled \|\| isRowLoading\(testingState, row\.id\)/);
  assert.match(aiServicesTableSource, /row\.isDisabled \|\| isRowLoading\(probingState, row\.id\)/);
});
