...
test("导入页应维护显式进度文案，避免长时间 AI 去重时只有处理中提示", () => {
  ...
  assert.match(pageComposableSource, /const differenceDialogFooterTip = computed\(\(\) =>/);
});

test("导入页在执行中应展示进度提示面板", () => {
  assert.match(confirmPanelSource, /v-if="importing" class="import-progress-panel"/);
...
test("导入按钮与重复确认按钮应复用更明确的进度文案", () => {
  ...
  assert.match(differenceDialogSource, /\{\{\s*confirmDifferenceButtonText\s*\}\}/);
  assert.match(differenceDialogSource, /\{\{\s*differenceDialogFooterTip\s*\}\}/);
  assert.match(pageComposableSource, /: `未确认 \$\{pendingUndecidedCount\.value\} 条`;/);
  assert.match(
    batchExecutionSource,
    /ElMessage\.warning\(\s*`请先逐条确认重复项（仍有 \$\{options\.pendingUndecidedCount\.value\} 条未确认）`\s*\);/
  );
  assert.match(differenceDialogSource, /<span>未确认 \{\{ pendingUndecidedCount \}\} 条<\/span>/);
});
...
test("导入页 AI 疑似重复默认配置应复用共享 helper", () => {
  assert.match(`${pageComposableSource}\n${batchExecutionSource}`, /createDefaultImportDuplicateAiConfig/);
  assert.match(
  ...
  assert.doesNotMatch(pageComposableSource, /importDuplicateAiConfig\.value = \{\s*enableSemanticDuplicateCheck: false/);
});
...
test("导入页应复用独立的聚合 helper，避免主文件继续堆积纯结果拼装逻辑", () => {
  ...
  assert.doesNotMatch(pageComposableSource, /const buildEmptyImportAggregate = \(\): CombinedImportResult =>/);
  assert.doesNotMatch(pageComposableSource, /const mergeImportAggregates = \(/);
  assert.doesNotMatch(pageComposableSource, /const createSingleTableAggregate = \(/);
...
test("导入页应把批量执行链路下沉到 composable，页面只保留交互编排", () => {
  ...
  assert.doesNotMatch(pageComposableSource, /const executeImportBatch = async \(/);
  assert.doesNotMatch(pageComposableSource, /const buildDuplicateCheckOptions = \(\)/);
  assert.doesNotMatch(pageComposableSource, /const validateDuplicateAiConfig = \(\)/);
  assert.doesNotMatch(pageComposableSource, /const buildImportProgressText = \(/);
});

test("导入页应复用差异展示格式化 helper，避免弹窗展示逻辑继续堆在页面里", () => {
  assert.match(differenceDialogSource, /from "\.\.\/dataImport\.difference-formatters";/);
  ...
  assert.doesNotMatch(pageComposableSource, /const getDifferenceMatchTypeLabel = \(/);
  assert.doesNotMatch(pageComposableSource, /const getDifferenceMatchTypeTagType = \(/);
  assert.doesNotMatch(pageComposableSource, /const hasAiDifferenceMeta = \(/);
  assert.doesNotMatch(pageComposableSource, /const isDifferenceFieldChanged = \(/);
  assert.doesNotMatch(pageComposableSource, /const differenceColumnDefs: DifferenceColumnDef\[\] = /);
  assert.doesNotMatch(pageComposableSource, /const isDifferenceColumnChanged = \(/);
});
