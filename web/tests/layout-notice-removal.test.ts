...
const navbarSource = readFileSync(
  resolve(process.cwd(), "web/src/layout/components/lay-navbar/index.vue"),
  "utf8"
);

const navHorizontalSource = readFileSync(
  resolve(process.cwd(), "web/src/layout/components/lay-sidebar/NavHorizontal.vue"),
  "utf8"
);

const navMixSource = readFileSync(
  resolve(process.cwd(), "web/src/layout/components/lay-sidebar/NavMix.vue"),
  "utf8"
);

const sidebarStyleSource = readFileSync(
  resolve(process.cwd(), "web/src/style/sidebar.scss"),
  "utf8"
);
...
test("通知组件目录应被移除", () => {
  assert.equal(
    existsSync(resolve(process.cwd(), "web/src/layout/components/lay-notice")),
    false
  );
});
